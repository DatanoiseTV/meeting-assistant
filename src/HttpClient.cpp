...
size_t HttpClient::WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) { ((std::string*)userp)->append((char*)contents, size * nmemb); return size * nmemb; }
HttpClient::Response HttpClient::post(const std::string& url, const json& payload, const std::map<std::string, std::string>& headers) {
    ...
    if (curl) {
        ...
        for (const auto& header : headers) { std::string h = header.first + ": " + header.second; chunk = curl_slist_append(chunk, h.c_str()); }
